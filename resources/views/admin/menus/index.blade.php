            ...
            <tbody class="divide-y divide-slate-800/50 text-sm text-slate-300">
                @forelse($menus as $menu)
                    <tr class="hover:bg-slate-800/10 transition-colors">
                        ...
                        <td class="py-4 px-4">
                            @if($menu->parent)
                                <span class="text-xs text-slate-400 bg-slate-800/40 px-2 py-1 rounded border border-slate-800">
                                    {{ $menu->parent->name }}
                                </span>
                            @else
                                <span class="text-xs text-slate-600 italic">None (Root)</span>
                            @endif
                        </td>
                        <td class="py-4 px-4">
                            @if($menu->permission_name)
                                <span class="text-xs font-mono text-amber-500 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded">
                                    {{ $menu->permission_name }}
                                </span>
                            @else
                                <span class="text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded">
                                    Public Access
                                </span>
                            @endif
                        </td>
                        <td class="py-4 px-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.menus.edit', $menu->id) }}" class="p-1.5 bg-indigo-600/10 text-indigo-400 hover:bg-indigo-600 hover:text-white rounded-lg border border-indigo-500/20 transition-all" title="Edit Menu">
                                ...
                                <form action="{{ route('admin.menus.destroy', $menu->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this menu item?')" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-1.5 bg-rose-600/10 text-rose-400 hover:bg-rose-600 hover:text-white rounded-lg border border-rose-500/20 transition-all" title="Delete Menu">
