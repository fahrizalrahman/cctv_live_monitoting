...
<div class="bg-[#0d1321]/30 border border-slate-800 rounded-3xl p-6 shadow-xl w-full">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h2 class="text-xl font-bold text-slate-200">Modify Access Role</h2>
            <p class="text-xs text-slate-500 mt-1">Configure role permissions and details for system authorization.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.roles.update', $role->id) }}" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" id="name" name="name" value="{{ old('name', $role->name) }}" required
                   ...
                   class="block w-full px-4 py-2.5 {{ $role->name === 'admin' ? 'bg-[#0a0e1a]/40 text-slate-500 cursor-not-allowed' : 'bg-[#0a0e1a]/80 text-slate-200 focus:ring-2 focus:ring-indigo-600 focus:border-transparent' }} border border-slate-800 rounded-xl placeholder-slate-600 focus:outline-none transition-all text-sm font-semibold tracking-wider lowercase" />
            @if($role->name === 'admin')
                <span class="block text-[10px] text-rose-500/80 mt-1">System administrator role name is locked.</span>
            @else
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="permission_{{ $permission->id }}" 
                                   ...
                                   {{ $role->name === 'admin' && $permission->name === 'manage roles' ? 'disabled' : '' }}
                            ...
                            @if($role->name === 'admin' && $permission->name === 'manage roles')
                                <!-- Keep field in form so it doesn't get omitted -->
