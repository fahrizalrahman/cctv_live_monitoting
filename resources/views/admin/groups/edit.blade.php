...
@section('page_title', 'Edit Group CCTV')

@section('content')
<div class="bg-[#0d1321]/30 border border-slate-800 rounded-3xl p-6 shadow-xl w-full">
        <div class="mb-6">
            <h2 class="text-xl font-bold text-slate-200">Edit Group</h2>
            <p class="text-xs text-slate-500 mt-1">Update CCTV group details.</p>
        </div>
        ...
        <form action="{{ route('admin.groups.update', $group->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full p-3 bg-[#0a0e1a]/80 border border-slate-800 rounded-xl text-sm text-slate-200 focus:ring-2 focus:ring-indigo-600 focus:border-transparent transition-all placeholder:text-slate-600">{{ old('description', $group->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                @enderror
            </div>
